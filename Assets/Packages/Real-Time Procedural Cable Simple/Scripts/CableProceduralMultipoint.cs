...
public class CableProceduralMultipoint : MonoBehaviour
{
    ...
    [
        SerializeField,
        Tooltip(
            "Series of points in cable.  If this transform is the desired start, add it to this array.")
    ]
    ...
    [
        SerializeField,
        Tooltip(
            "Number of points per unit length, using the straight line from the start to the end transform.")
    ]
    ...
    void Start()
    {
        ...
        {
            Debug
                .LogError("No sections assigned to Cable_Procedural component attached to " +
                gameObject.name);
            return;
        }
    ...
    void Draw(CableSection section)
    {
        ...
        Vector3 vectorFromStartToEnd =
            section.end.position - section.start.position;
        ...
        int pointsCount =
            Mathf.FloorToInt(pointDensity * vectorFromStartToEnd.magnitude);
        ...
        while (i < pointsCount)
        {
            ...
            float pointForCalcs =
                (float) i / ((pointsCount - 1) > 0 ? pointsCount - 1 : 1);
